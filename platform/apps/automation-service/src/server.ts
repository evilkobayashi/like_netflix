...
const queue = new Queue('automation-execution', {
...
async function handleEvent(event: DomainEvent): Promise<void> {
  ...
  await processDomainEvent(prisma, event);
...
app.get('/automation/rules', async (request) =>
  prisma.automationRule.findMany({ where: { tenantId: (request.user as { tenantId: string }).tenantId } })
);

app.post('/automation/rules', async (request) => {
  const tenantId = (request.user as { tenantId: string }).tenantId;
  const body = request.body as { name: string; trigger: string; action: Record<string, unknown>; active?: boolean };
...
app.get('/automation/executions', async (request) =>
